...
void FP_connet_the_words(char* sorted_str, char** words, int count) {
	...
	for (int i = 0; i < count; i++) {
		...
		pos += strlen(words[i]);
		int nextPos = pos;
		while (sorted_str[nextPos] == ' ') { // додати наступний символ, якщо це не роздільник
			sorted_str[pos] = sorted_str[nextPos];
			pos++;
			nextPos++;
		}
		sorted_str[pos] = ' '; // додати пробіл після слова
		pos++;
	}
	sorted_str[pos - 1] = '\0'; // замінити останній пробіл нуль-символом
...
void FP_sort_words_by_length(char* str) {
	...
	char* words[100];
	char sorted_str[200];
	int count = FP_split_string(str, words, 100); // розбиваю рядок на слова
	FP_insertion_sort(words, count); // вісдортувати слова
	FP_connet_the_words(sorted_str, words, count); // знову роблю рядок зі слів
